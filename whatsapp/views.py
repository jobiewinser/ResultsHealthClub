...
def whatsapp_number_change_alias(request):
    ...
    if get_profile_allowed_to_edit_whatsapp_settings(request.user.profile, whatsappnumber.whatsapp_business_account.first().site.company):
...
def send_new_template_message(request):
    ...
    if not lead:
        lead = Campaignlead.objects.filter(campaign__site=whatsappnumber.whatsapp_business_account.first().site, whatsapp_number=combined_number).first()
